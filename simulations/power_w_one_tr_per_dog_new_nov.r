...
re2sim=fe2sim
fe2sim["(Intercept)"]=qlogis(0.5)
fe2sim["belieftrue"]=qlogis(0.3)-qlogis(0.5)
...
prob.outs.c=0.4
n.breeds=8
save.image("simulations/power_w_one_tr_per_dog_new_nov.RData")
load("simulations/power_w_one_tr_per_dog_new_nov.RData")
...
for(n in 1:length(n.per.group)){
	...
	save(file=paste(c("simulations/sim_files/lucrezia_bwsimwbreed_", n.per.group[n], ".RData"), collapse=""), list="i.res")
	print(paste(c(n.per.group[n], max(n.per.group)), collapse=" out of "))
}


flist=list.files(path="simulations/sim_files", full.names=T)
all.res=gsub(x=flist, pattern="simulations/sim_files/lucrezia_bwsimwbreed_", replacement="", fixed=T)
...
legend("topright", legend=c("full: error", paste(c("full", "belief", "sex", "baited.fst", "age"), "warrning", sep=": ")),
	pch=c(2, pchs))
savePlot(file="simulations/graphs/problems.png", type="png")
...
savePlot(file="simulations/graphs/power.png", type="png")
...
for(i in 1:ncol(all.fe.range)){
	...
	xx=gsub(xx, pattern=")", replacement="", fixed=T)
	xx=gsub(xx, pattern=".", replacement="", fixed=T)
	savePlot(file=paste(c("simulations/graphs/FE_", xx, ".png"), collapse=""), type="png")
}
# 	
# re2sim.r=re2sim
