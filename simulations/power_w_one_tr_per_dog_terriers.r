...
source("simulations/functions/drop1_para.r")
...
re2sim=fe2sim
fe2sim["(Intercept)"]=qlogis(0.7)
fe2sim["belieftrue"]=qlogis(0.3)-qlogis(0.7)
re2sim["(Intercept)"]=0.3
re2sim["belieftrue"]=0.3

n.per.group=seq(4, 10, by=1)
age.range.t=c(5/12, 13)
age.range.c=c(6, 10)
prob.outs.c=0.4
n.breeds=12
save.image("simulations/power_w_one_tr_per_dog_terriers.RData")
load("simulations/power_w_one_tr_per_dog_terriers.RData")
...
for(n in 1:length(n.per.group)){
	...
	save(file=paste(c("simulations/sim_files/terriers/lucrezia_bf_terriers_", n.per.group[n], ".RData"), collapse=""), list="i.res")
	print(paste(c(n.per.group[n], max(n.per.group)), collapse=" out of "))
}


flist=list.files(path="simulations/sim_files/terriers/", full.names=T)
all.res=gsub(x=flist, pattern="simulations/sim_files/terriers/lucrezia_bf_terriers_", replacement="", fixed=T)
...
legend("topright", legend=c("full: error", paste(c("full", "belief", "sex", "baited.fst", "age"), "warrning", sep=": ")),
	pch=c(2, pchs))
savePlot(file="simulations/graphs/terriers/problems.png", type="png")
...
savePlot(file="simulations/graphs/terriers/power.png", type="png")
...
for(i in 1:ncol(all.fe.range)){
	...
	xx=gsub(xx, pattern=")", replacement="", fixed=T)
	xx=gsub(xx, pattern=".", replacement="", fixed=T)
	savePlot(file=paste(c("simulations/graphs/terriers/FE_", xx, ".png"), collapse=""), type="png")
}
# 	
# re2sim.r=re2sim
...
# 	xx=gsub(xx, pattern=")", replacement="", fixed=T)
# 	xx=gsub(xx, pattern=".", replacement="", fixed=T)
# 	savePlot(file=paste(c("simulations/graphs/RE_", xx, ".png"), collapse=""), type="png")
# }
