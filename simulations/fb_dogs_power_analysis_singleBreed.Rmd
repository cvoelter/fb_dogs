...
#load("fb_simulation_single_breed_based_on_exp1_n60.RData")
...
fb.per <- c(0.3, 0.5) # performance in fb condition
tb.per <- c(0.3, 0.7) # performance in tb condition
...
n.simus <- 10000 # small number for testing
...
save.image("fb_simulation_single_breed_based_on_exp1_N64.RData")
```
...
p.con.power <- ggplot(data = lrt.data2, aes(x= as.factor(fb.per),y = proportion.sign.lrt.con, fill=as.factor(tb.per))) +
  ...
  labs(fill = "TB condition", y="Power", x= "FB condition") +
  theme_few()#+
  #theme(legend.position="none")
p.con.power

ggsave(p.con.power, filename = "../graphs/single_breed_simulation_N64.png", scale = 0.8, height = 5, width = 7)
```