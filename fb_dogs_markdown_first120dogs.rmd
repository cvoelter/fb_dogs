...
p.choice.fci<-ggplot(
  data=plot.data.fci, aes(x=condition, y=sum.correct)) +
  geom_bar(stat="identity", alpha=0.5)+
  #geom_errorbar(aes(ymin=mean.correct-se.correct, ymax=mean.correct+se.correct), width=.2) +
  #ylim(0,1)+
  theme_classic()+
...
plot.data.ant2 <- fb_data %>%
  mutate(condition=recode(condition, "FalseBelief"="False Belief", "TrueBelief"="True Belief"))%>%#renaming of factor levels
  mutate(prop_lt_buckets=prop_lt_baited_pause+prop_lt_empty_pause)
...
p.ant.con2<-ggplot(
  ...
  theme_classic()+
  labs(x="Condition",y="")
plot_grid(p.ant.con2_wo_zeros, p.ant.con2, labels=c("A", "B"))
```
